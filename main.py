...
async def get_sample_dataset():
    """Returns synthetic hiring data with intentional gender bias."""
    np.random.seed(42)
    n = 500
...
async def analyze_fairness(req: AnalysisRequest):
    ...
    try:
        ...
        if len(privileged_group) == 0 or len(unprivileged_group) == 0:
            return {"insufficient_data": True, "message": "Insufficient data for one or both groups."}
        ...
        return {
            ...
            "fairness_score": int(max(0, 100 - abs(dp_gap * 100)))
        }
